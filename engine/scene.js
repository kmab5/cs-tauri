...
// Legacy compatibility: games released before implicit control flow became
// mandatory fall out of *choice options without *goto. Default false keeps
// those games playable; set true to surface the modern authoring error.
Scene.strictChoiceEnd = false;
Scene.warnedChoiceFallthrough = {};
...
Scene.prototype.printLoop = function printLoop() {
    ...
    for (;!this.finished && this.lineNum < this.lines.length; this.lineNum++) {
        ...
        if (choiceEnd === 0) {
            ...
            //
            // choiceEnd === 0 means "this #option ended without *goto/*finish
            // and implicit_control_flow is off". Engines before implicit
            // control flow became mandatory had no such sentinel: execution
            // simply continued onto the following line. Games released against
            // those engines (which never set implicit_control_flow) depend on
            // that, so throwing here makes them unplayable.
            //
            // Default is the legacy behaviour. Set Scene.strictChoiceEnd = true
            // to get the modern error instead — useful when authoring new games,
            // where falling out of a choice is almost always a mistake.
            if (Scene.strictChoiceEnd) {
            ...
            if (!Scene.warnedChoiceFallthrough[this.name + ':' + this.lineNum]) {
                Scene.warnedChoiceFallthrough[this.name + ':' + this.lineNum] = true;
                if (typeof console !== "undefined" && console.warn) {
                    console.warn(this.lineMsg() + "falling out of a *choice without *goto or *finish (legacy behaviour)");
                }
            }
            // fall through, exactly as the old engine did
        } else if (choiceEnd) {
