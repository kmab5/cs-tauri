/*
 * VENDORED FROM UPSTREAM — DO NOT EDIT.
 *
 * dfabulich/choicescript · editor/embeddable-autotester.js
 * Copyright 2010 by Dan Fabulich, under the ChoiceScript License 1.0
 * https://www.choiceofgames.com/LICENSE-1.0.txt
 *
 * This is upstream's own quicktest, unmodified. `quicktest.js` in that repo is a
 * four-line shim over `autotest.js`, which loads *this* file — the embeddable
 * form the ChoiceScript web editor uses. It walks every branch of a scene by
 * cloning the interpreter, and returns [coverage, uncovered].
 *
 * It is vendored rather than reimplemented for the reason the project has held
 * to throughout: the engine is upstream's, and a test that is not upstream's
 * test does not tell you what upstream's test would.
 *
 * It expects three globals, which src/lib/author/headless.ts provides: Scene,
 * nav, stats. It stubs its own printx/printParagraph and logs through console.
 */
